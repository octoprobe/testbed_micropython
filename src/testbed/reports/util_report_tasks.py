...
from testbed.reports.util_report_renderer import (
    Align,
    RendererAscii,
    RendererBase,
    RendererMarkdown,
...
class ReportTentacle:
    ...
    def __hash__(self) -> int:
        return hash(self.label + "," + self.board_variant)
...
class Task:
    ...
    end_s: float
    label: str
    tentacles: list[ReportTentacle] = dataclasses.field(default_factory=list)
    """
...
class Tasks(list[Task]):
    def quantize_times(self) -> None:
        def quantize(time_s: float) -> float:
            quantize_factor = 5  # 200ms
            return int(time_s * quantize_factor) / quantize_factor
